...
#ifndef _SPI_DMA_SEND_H_INCLUDED
#define _SPI_DMA_SEND_H_INCLUDED
...
class SPISettings {
  ...
  private:
    ...
    friend class SPIDMAClass;
};

class SPIDMAClass {
  public:
    SPIDMAClass();
    SPIDMAClass(uint8_t mosi, uint8_t miso, uint8_t sclk, uint8_t ssel = (uint8_t)NC);
	...
	void send(uint8_t pin, void *_buf, size_t _count, SPITransferMode _mode = SPI_LAST);
	
	void send(void *_buf, size_t _count, SPITransferMode _mode = SPI_LAST)
    {
      send(CS_PIN_CONTROLLED_BY_USER, _buf, _count, _mode);
    }
...
extern SPIDMAClass SPIDMA;

#endif /* _SPI_DMA_SEND_H_INCLUDED */
